...
waveforms, which clearly put the original IMRPhemonD result in a 
disadvantage. We have tried to train the model using a set of training waveforms of the same 
intrinsic parameters, but with BAM waveforms replaced with SEOBNR waveforms. The result 
shows a marginal improvement, which corresponds to the stitching problem discussed before.
However, it is known that IMRPhenomD has a large inaccuracy in the large mass-ratio regime, 
and is rarely used in this region of parameter space. Hence, we focus on showing AD with 
gradient descent, as a general and robust method, can be used to further optimize models 
independent of the training waveforms used.}
