...
In this section, we first briefly review the contruction of the IMRPhenomD model and discuss
...
Interested readers should refer to \citep{khan2016frequency} for more details on construction of the model.
...
\end{align}
\te{Check minus sign}
Here $f$ is the frequnecy, $\theta$ are of intrinsic parameters of the binary, and $\Lambda$ is a set of additional
...
PhenomD uses the TaylorF2 model \citep{boyle2007high} which is known up to 3.5PN order.
...
Thus, having a set of accurate waveform coefficients is crucial to the accuracy
of the GW models. 
...
At the time of construction this piece-wise fitting was necessary since
$\lambda$ has 209 components making the optimization computationally prohibitive.
Here, we recalibrate the $\lambda$ coefficients jointly. 
This is made possible by the use of gradient-based optimization algorithms which are
enabled by AD from \jax and {\ripple}.
