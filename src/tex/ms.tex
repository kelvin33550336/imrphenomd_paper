...
Due to the number of calibration parameters (often in a couple hundreds if not
...
calibration coefficients given a set of NR waveforms. Training on a small number
of NR waveforms, we demonstrate one can improves the match between IMRPhenomD
and NR waveforms over a decently sized parameter space, up to mass ratio $q=8$
We also explore how different parts of the source parameter space (e.g. the
primary and second spins) respond to the optimization procedure, by optimizing
the waveform separately for different regions of the parameter space. This gives
us hints on whether the waveform model performs equally well in different
...
axis, we can see the waveforms with $q\lq4$ show most consistent average
...
top of that, in the $q\lq4$ region, there seems to be more consistent
