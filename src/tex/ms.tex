...
waveforms from the catalog presented in \citep{khan2016frequency}.\te{Which one? Please write the name from the table.}
...
magnitude, and a median mismatch being reduced by 50\% (see vertical dotted lines). When using
...
Note that the IMRPhenomD model was initally tested using the
{\zdethp} weighted mismatch. We would therefore like to examine whether using the {\zdethp} PSD in our loss function
...
On the spin axis, we can see the waveforms with
...
Since the original waveform was developed for aligned-spin systems, the waveforms
...
Waveforms along the diagonal axis, $\chi_1\sim\chi_2$, show good mismatch improvements as discussed
...
region, waveforms generally improve slightly after optimization. However,
waveforms in the bottom-right region do not improve after optimization. 
...
are calculated using simple average of the mismatches, $\mathcal{L}_{\mathrm{ave}}$. The top-left and
...
further improve on top of the original IMRPhenomD waveform. 
...
\begin{figure}[t]
	...
	(Bottom) regions of Fig.~\ref{fig:ps_q148_quadrant}. We use a constant noise spectrum to calculate mismatch and
