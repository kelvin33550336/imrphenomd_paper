...
estimation~\citep{Dax:2021tsq, Islam:2022afg, zackay2018relative, Ashton:2018jfp}, rely upon accurate waveform
...
Here we adopt a quantity commonly used in GW physics called the \textit{mismatch} function \citep{Owen:1995tm,husa2016frequency}. 
It is defined as
...
behaviour during the optimization process.
...
and preferencially optimize the largest mismatches, encouraging the waveform to have similar mismatches everywhere.
...
For this, we taper the time-series using Tukey window~\citep{tukey}\footnote{
...
\begin{figure}
	...
	\caption{Loss functions against the number of iterations.
	The vertical dotted line indicates minimum of the validation loss,
	where we stop the optimization and obtain a new set of waveform coefficients, $\lambda$.}
	\label{fig:loss}
\end{figure}
...
\begin{figure}[t]
	...
	\caption{Distribution of training and validation NR waveforms in the $q-\chi_{PN}$
	space. The blue stars represent the waveforms used to compute the training loss and the orange
	dots represent the waveforms used for validation.\kw{Change the legend, and include the extra points. Change the training point to be star and testing point to be dot.}}
...
\begin{table}[t]
	...
	here, nine of them are also used in the original IMRPhenomD calibration. The remaining two
	are SXS analogues to two of the BAM NR simulations used in the original calibration
...
To illustrate the effect of optimiziation on an individual waveform level, in Fig.
...
$\mathcal{L}_{\mathrm{nl}}$, we observe a less pronounced improvement with a 22.9\%
...
Note that the performace of the IMRPhenomD model was initally tested using the
...
could lead to an improved to mismatch. Peforming the optimization again, we find no significant difference
...
We can see that waveforms with $q\le4$ show the most consistent average improvement.
...
This is particularly true in the $q\le4$ region, where we see a consistent
improvement of the waveform for $\chi_{\textrm{PN}}<0$. 
Since the original model was developed for aligned-spin systems, NR waveforms
with opposite sign $\chi_{1,2}$ are likely less well-fit \kw{Verify this.}.
To show this, we plot the parameter space $\chi_1 - \chi_2$ in Fig.~\ref{fig:ps_q148}. 
...
($\chi_1>0>\chi_2$) regions respond to the optimization differently. In the top-left
...
\begin{table}[t]
	...
	\end{tabularx}
	\caption{List of NR waveforms used in recalibrating the coefficients in the four $\chi_1-\chi_2$ quadrants.
	From top to bottom the lines denote the top-right ($\chi_1,\chi_2>0$), top-left ($\chi_1<0<\chi_2$),
	bottom-left ($\chi_1,\chi_2<0$), and bottom-right ($\chi_1>0>\chi_2$) regions.
...
equal-spin waveforms lying on the diagonal improve the most. The
improvement above the diagonal is also significantly more than when optimizing using
all waveforms at once. This can be seen clearly in the top panel of Fig.~\ref{fig:all_quadrants}, where we see
that the optimized histogram shifts to lower mismatches.\footnote{
...
improvement below the diagonal is negligible, indicating that our method struggles to
further improve over the original IMRPhenomD waveform. 
Interestingly, we can see in the bottom-right quadrant that the waveform does not show a significant change
...
\begin{figure}[t]
	...
	Each quadrant is fitted independently with to the NR waveforms listed in Tab.~\ref{tab:quadrants}.}
...
% As discussed in Sec.~\ref{sec:method} our recalibration procedure is not exactly the same as the original
% calibration. For instance, we use a different set of NR waveforms, frequency
% ranges, etc. Nonetheless, since the decrease in mismatch is rather significant, we believe that
% the high-dimensional optimization procedure is primarily responsible to the improved performance.
...
We therefore encourage the waveform development community to utlize these tools
and hope that this work can effetively contribute to the development of accurate waveform models.
...
give additional features that can test and explain IMRPhenomD better. \te{I don't really
understand this paragraph. Should we remove it?}
