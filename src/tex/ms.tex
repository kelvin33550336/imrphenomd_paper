...
as \rr{matched} filtering~\citep{owen1996search, owen1999matched} and parameter 
...
\end{align}
where $f$ is the frequency, $\theta$ are the intrinsic parameters of the binary, and $\Lambda^i$ \rr{($i=1,2,\cdots,19$) is a set of 19 additional
...
to 3.5PN order \kl{augmented with the next four higher order PN terms}.
...
\begin{figure}
	...
	\caption{\rr{Average Loss $\mathcal{L}_{\mathrm{ave}}$} against the number of iterations. The vertical
...
We note here that the mismatch can be viewed as \rr{a} mean square error (MSE) between the
two waveforms.
...
$\mathcal{M}_{i,\mathrm{ini}}$ represents the mismatch \rr{between original IMRPhenomD waveforms and NR waveforms}, and $N$ is the total number of training waveforms.

The two loss functions are chosen to give different 
behavior during the optimization process.
The simple average, \rr{$\mathcal{L}_{\mathrm{ave}}$}, serves as the simplest choice of loss function,
...
Conversely, \rr{$\mathcal{L}_{\mathrm{ave}}$} allows the loss function to automatically adjust
...
magnitude. The median mismatch is reduced by 50\% (see vertical dotted lines) \kl{while the tail hardly changes}. When using
...
positive and $\chi_{2}$ is negative. \kl{\sout{This means there is a dependency between
...
\kl{We note that in comparison to the original IMRPhenomD calibration, we restricted the 
parameter space to $q\leq8$ and directly used NR waveform instead of hybridized SEOB-NR 
waveforms in training and validation purposes. These choices put the original IMRPhemonD 
result in a disadvantage, as both the parameter space and frequency range are restricted. 
We tested our result by training the model using a set of training waveforms of the same 
intrinsic parameters, but with BAM waveforms replaced with SEOBNR waveforms. We can see 
the result in Fig.~\ref{fig:q14818}, where the mismatch median changes by around $8\%$, 
This improvement mainly corresponds to the stitching problem discussed before. 
...
AD with gradient descent, as a general and robust method, can be used to further optimize 
models independent of the training waveforms used.} 
