...
\kl{Both top-right and bottom-left region have good performance comparing with the original model. Especially for the bottom-left region, where the improvement is significantly better than optimizing all regions simultaneously. This suggests the ansatz are well-fitted to this part of the parameter space.
Waveforms above the diagonal also give good improvements when comparing with the original model and it has a similar improvement as optimizing all waveforms at once. Notice optimizing all regions gives a slightly better improvement. This shows that the ansatz can achieve a better fit using training waveforms with similar spins, even for waveforms with non-equal spin parameters.}
...
\kl{Overall, the improvement for both optimization schemes are similar (bottom panel of Fig.~\ref{fig:ps_q148_quadrant}). Although split region optimization uses more training waveforms, some of the waveforms, e.g. opposite spins, hinders the optimization procedure and hence gives a slightly worse result when comparing with all region optimization.}
