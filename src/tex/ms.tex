...
the relevant maximum and minimum frequencies for the integration. $t_0$ and $\phi_0$ are maximized by performing regression between $h_1$ and $h_2$. The mismatch
...
recalibration procedure on each region separately (Fig.~\ref{fig:ps_q148_quadrant}). The
...
waveforms lying in the lower diagonal have mismatches worsened while other waveforms 
generally improve. For the top-right and bottom-left regions, we see that the optimization prefers waveforms with $\chi_1<\chi_2$. This further  In the top-left region, the improvement is significant, except for a
few defects caused by some testing waveforms having $q>4$. This can be seen clearer in 
Fig.~\ref{fig:all_quadrants}, which we see that the optimized histogram shifts downward 
rather uniformly. The optimized waveforms in the bottom-right region have two distinct 
features. Within this region, waveforms in the bottom-right corner improves significantly 
while others worsened. This phenomenon can also be seen in Fig.~\ref{fig:all_quadrants}, 
where the mismatch distribution shifted upward. Hence, we can deduce that the loss function 
descended into a different minimum compared to Fig.~\ref{fig:ps_q148}, making the optimized 
coefficients not useful in GW analysis. THus, further optimizing in a smaller region does 
not generally improve the results.
%\kw{I am a bit confused about why you can get worse result.}
...
\begin{figure}[t]
	...
	$\mathcal{L}_{\mathrm{mean}}$ as the loss function. Waveforms in the top-left region generally improves while waveforms in the bottom-right region worsened, as indicated by the median (dashed lines).}
...
\begin{figure}[t]
	...
	\caption{Parameter space of testing waveforms. Each region is fitted independently with waveforms listed in Tab.~\ref{tab:quadrants}. Colorbar represents the $\log_{10}$ difference of mismatches before and after optimization.}
...
performance has improved, while the bottom-right region exhibits two distinct 
features. This is because the loss function has descended into a different 
minimum which highly favors events with $\chi_1\approx1$ and $\chi_2\approx-1$. 
This observation suggests the ansatz is specific to certain regions of the 
parameter space, with a preference for BBH events lying in the upper diagonal 
region, and it hardly fits with waveforms lying in the lower diagonal region. 
...
improvement to the top-left region while the bottom-right region falls into a different 
loss minimum. This indicates that the ansatz does not fit certain waveforms in this 
region, hence introduces bias to downstream GW analysis. This phenomenon is mainly 
due to the reduced spin approximation used in parameterizing the 
